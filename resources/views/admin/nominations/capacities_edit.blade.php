...
<div class="container py-3">
  <h5 class="mb-3">全大会の団体別割当一覧</h5>

  @if(session('status'))
    <div class="alert alert-info">{{ session('status') }}</div>
  @endif

  {{-- コントローラ側で route を用意していれば差し替えてください --}}
  <form method="POST" action="{{ route('admin.nominations.capacities.bulkUpdate') }}">
    @csrf
    @method('PATCH')
              ...
              <tr>
                ...
                @foreach ($visibleOrgs as $org)
                  ...
                  <td class="text-center" style="width: 20px">
                    <input
                      type="number" min="0"
                      class="form-control form-control-sm d-inline-block text-center cap-input"
                      name="capacity[{{ $nom->id }}][{{ $org->id }}]"
                      value="{{ $val }}"
                      data-row="row-{{ $nom->id }}"
                    >
                  </td>
